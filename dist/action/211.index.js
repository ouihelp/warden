...
function buildSkillCheckPayload(report, options) {
    ...
    const findingsConclusion = options.conclusion ?? determineConclusion(filteredForConclusion, options.failOn, options.failCheck);
...
async function createCompletedCoreCheck(octokit, summaryData, conclusion, options) {
    ...
    const title = summaryData.title ?? (incomplete
        ? 'Analysis incomplete'
...
function buildSkillSummary(report) {
    const incomplete = reportIsIncomplete(report);
    const lines = [incomplete ? 'Analysis incomplete.' : (0,utils/* escapeHtml */.ZD)(report.summary), ''];
    const incompleteReasons = [];
    if (report.failedHunks) {
        incompleteReasons.push(`${report.failedHunks} chunk${report.failedHunks === 1 ? '' : 's'} failed to analyze`);
    }
    if (report.failedExtractions) {
        incompleteReasons.push(`${report.failedExtractions} finding extraction${report.failedExtractions === 1 ? '' : 's'} failed`);
    }
    if (incompleteReasons.length > 0) {
        lines.push(`⚠️ Analysis incomplete: ${incompleteReasons.join('; ')}.`, '');
    }
    if (report.findings.length === 0) {
        lines.push(incomplete ? 'No findings were reported by the completed chunks.' : 'No issues found.', '');
    }
    else {
...
function buildCoreSummary(data) {
    ...
    if (topFindings.length > 0) {
        ...
        lines.push(data.message
            ? (0,utils/* escapeHtml */.ZD)(data.message)
...
function coreAnalysisIsIncomplete(data) {
...
const FindingsOutputSchema = schemas/* object */.Ik({
    ...
    skills: schemas/* array */.YO(schemas/* object */.Ik({
        ...
        hunkFailures: schemas/* array */.YO(types/* HunkFailureSchema */.se).optional(),
        error: types/* SkillErrorSchema */.J1.optional(),
        verifierRejections: types/* VerifierRejectionsSchema */.IH.optional(),
