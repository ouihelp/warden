...
/* harmony export */ });
/* harmony import */ var _models_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(68314);
/* harmony import */ var _utils_hash_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(65078);
/* harmony import */ var _utils_json_parse_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(21034);
/* harmony import */ var _utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(30883);
/* harmony import */ var _constrained_sampling_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(25155);
/* harmony import */ var _transform_messages_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(75702);
...
function convertToolResultOutput(model, content) {
    ...
    if (images.length === 0 || !model.input.includes("image")) {
        return (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_0__/* .sanitizeSurrogates */ .J)(hasText ? textResult : images.length > 0 ? "(see attached image)" : "(no tool output)");
    }
    const output = [];
    if (hasText) {
        output.push({ type: "input_text", text: (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_0__/* .sanitizeSurrogates */ .J)(textResult) });
    }
...
function convertResponsesMessages(model, context, allowedToolCallProviders, options) {
    ...
    const buildForeignResponsesItemId = (itemId) => {
        const normalized = `fc_${(0,_utils_hash_js__WEBPACK_IMPORTED_MODULE_1__/* .shortHash */ .B)(itemId)}`;
    ...
    const transformedMessages = (0,_transform_messages_js__WEBPACK_IMPORTED_MODULE_2__/* .transformMessages */ .b)(context.messages, model, normalizeToolCallId);
    ...
    if (includeSystemPrompt && context.systemPrompt) {
        ...
        messages.push({
            role,
            content: (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_0__/* .sanitizeSurrogates */ .J)(context.systemPrompt),
        });
    }
    let msgIndex = 0;
    for (const msg of transformedMessages) {
        if (msg.role === "user") {
            if (typeof msg.content === "string") {
                messages.push({
                    role: "user",
                    content: [{ type: "input_text", text: (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_0__/* .sanitizeSurrogates */ .J)(msg.content) }],
                });
            }
            else {
                const content = msg.content.map((item) => {
                    if (item.type === "text") {
                        return {
                            type: "input_text",
                            text: (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_0__/* .sanitizeSurrogates */ .J)(item.text),
                        };
                    }
                    return {
                        type: "input_image",
            ...
            for (const block of msg.content) {
                if (block.type === "thinking") {
                    ...
                    if (!msgId) {
                        ...
                        msgId = `msg_${(0,_utils_hash_js__WEBPACK_IMPORTED_MODULE_1__/* .shortHash */ .B)(msgId)}`;
                    }
                    output.push({
                        type: "message",
                        role: "assistant",
                        content: [{ type: "output_text", text: (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_0__/* .sanitizeSurrogates */ .J)(textBlock.text), annotations: [] }],
                        status: "completed",
                        id: msgId,
                    ...
                    if (customInputProperty !== undefined) {
                        output.push({
                            ...
                            call_id: callId,
                            name: toolCall.name,
                            input: (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_0__/* .sanitizeSurrogates */ .J)((0,_constrained_sampling_js__WEBPACK_IMPORTED_MODULE_3__/* .getGrammarToolInput */ .gY)(toolCall.name, toolCall.arguments, customInputProperty)),
            ...
            if (deferredTools.length > 0 && options?.deferredToolsMode === "additional-tools") {
                ...
                const searchCallId = `pi_tool_load_${(0,_utils_hash_js__WEBPACK_IMPORTED_MODULE_1__/* .shortHash */ .B)(`${msg.toolCallId}:${names.join(",")}`)}`;
                messages.push({
...
function convertResponsesTools(tools, options) {
    ...
    return tools.map((tool) => {
        const grammar = (0,_constrained_sampling_js__WEBPACK_IMPORTED_MODULE_3__/* .resolveGrammarConstrainedSampling */ .TH)(tool, supportsOpenAIGrammarTools);
        if (grammar) {
            return {
                type: "custom",
                name: tool.name,
        ...
        const constrainedStrict = (0,_constrained_sampling_js__WEBPACK_IMPORTED_MODULE_3__/* .resolveJsonSchemaStrictSampling */ ._F)(tool, supportsStrictMode);
        const strict = constrainedStrict ?? defaultStrict;
        const functionTool = {
            type: "function",
            name: tool.name,
            description: tool.description,
            parameters: (0,_constrained_sampling_js__WEBPACK_IMPORTED_MODULE_3__/* .getJsonSchemaToolParameters */ .Km)(tool, strict === true),
            ...(options?.deferLoading ? { defer_loading: true } : {}),
        };
...
async function processResponsesStream(openaiStream, output, stream, model, options) {
    ...
    const finalizeResponse = (response) => {
        ...
        (0,_models_js__WEBPACK_IMPORTED_MODULE_4__/* .calculateCost */ .yN)(model, output.usage);
    ...
    for await (const event of openaiStream) {
        if (event.type === "response.created") {
            ...
            slot.block.arguments = (0,_utils_json_parse_js__WEBPACK_IMPORTED_MODULE_5__/* .parseStreamingJson */ .o2)(slot.block.partialJson);
            ...
            if (item.type === "reasoning" && slot?.type === "thinking") {
                ...
                slot.block.arguments = (0,_utils_json_parse_js__WEBPACK_IMPORTED_MODULE_5__/* .parseStreamingJson */ .o2)(item.arguments || slot.block.partialJson || "{}");
