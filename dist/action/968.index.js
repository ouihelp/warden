...
/* harmony import */ var _utils_hash_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(65078);
/* harmony import */ var _utils_headers_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(56814);
/* harmony import */ var _utils_json_parse_js__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(21034);
/* harmony import */ var _utils_pi_user_agent_js__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(11213);
/* harmony import */ var _utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(30883);
/* harmony import */ var _constrained_sampling_js__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(25155);
/* harmony import */ var _simple_options_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(51821);
/* harmony import */ var _transform_messages_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(75702);
...
const stream = (model, context, options) => {
    const stream = new _utils_event_stream_js__WEBPACK_IMPORTED_MODULE_0__/* .AssistantMessageEventStream */ .Q2();
    (async () => {
        ...
        try {
            ...
            const transformedMessages = (0,_transform_messages_js__WEBPACK_IMPORTED_MODULE_1__/* .transformMessages */ .b)(context.messages, model, (id) => normalizeMistralToolCallId(id));
...
const streamSimple = (model, context, options) => {
    ...
    const base = {
        ...(0,_simple_options_js__WEBPACK_IMPORTED_MODULE_2__/* .buildBaseOptions */ .QP)(model, context, options, apiKey),
        toolChoice: options?.toolChoice,
    };
    const clampedReasoning = options?.reasoning ? (0,_models_js__WEBPACK_IMPORTED_MODULE_3__/* .clampThinkingLevel */ .Kt)(model, options.reasoning) : undefined;
...
function deriveMistralToolCallId(id, attempt) {
    ...
    return (0,_utils_hash_js__WEBPACK_IMPORTED_MODULE_4__/* .shortHash */ .B)(seed)
        .replace(/[^a-zA-Z0-9]/g, "")
...
function buildMistralHeaders(model, apiKey, options) {
    const headers = new Headers({
        "User-Agent": (0,_utils_pi_user_agent_js__WEBPACK_IMPORTED_MODULE_6__/* .getPiUserAgent */ .w)(),
...
function buildChatPayload(model, context, messages, options) {
    ...
    if (context.systemPrompt) {
        payload.messages.unshift({
            role: "system",
            content: (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_7__/* .sanitizeSurrogates */ .J)(context.systemPrompt),
        });
    }
    return payload;
}
...
async function consumeChatStream(model, output, stream, mistralStream) {
    ...
    for await (const event of mistralStream) {
        ...
        if (delta.content !== null && delta.content !== undefined) {
            ...
            for (const item of contentItems) {
                if (typeof item === "string") {
                    const textDelta = (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_7__/* .sanitizeSurrogates */ .J)(item);
                ...
                if (item.type === "thinking") {
                    const deltaText = (item.thinking ?? [])
                        ...
                        .join("");
                    const thinkingDelta = (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_7__/* .sanitizeSurrogates */ .J)(deltaText);
                    if (!thinkingDelta)
                        continue;
                ...
                if (item.type === "text") {
                    const textDelta = (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_7__/* .sanitizeSurrogates */ .J)(item.text ?? "");
                    if (!currentBlock || currentBlock.type !== "text") {
                        finishCurrentBlock(currentBlock);
                        currentBlock = { type: "text", text: "" };
                        output.content.push(currentBlock);
        ...
        for (const toolCall of toolCalls) {
            ...
            block.arguments = (0,_utils_json_parse_js__WEBPACK_IMPORTED_MODULE_8__/* .parseStreamingJson */ .o2)(block.partialArgs);
            stream.push({
                type: "toolcall_delta",
    ...
    for (const index of toolBlocksByKey.values()) {
        ...
        toolBlock.arguments = (0,_utils_json_parse_js__WEBPACK_IMPORTED_MODULE_8__/* .parseStreamingJson */ .o2)(toolBlock.partialArgs);
...
function toFunctionTools(tools) {
    return tools.map((tool) => {
        const strict = (0,_constrained_sampling_js__WEBPACK_IMPORTED_MODULE_9__/* .resolveJsonSchemaStrictSampling */ ._F)(tool, true);
        return {
            type: "function",
            function: {
                name: tool.name,
                description: tool.description,
                parameters: stripSymbolKeys((0,_constrained_sampling_js__WEBPACK_IMPORTED_MODULE_9__/* .getJsonSchemaToolParameters */ .Km)(tool, strict)),
                strict: strict ?? false,
            },
        };
    });
    ...
    for (const msg of messages) {
        if (msg.role === "user") {
            if (typeof msg.content === "string") {
                result.push({ role: "user", content: (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_7__/* .sanitizeSurrogates */ .J)(msg.content) });
                continue;
            }
            ...
            const content = msg.content
                ...
                .map((item) => {
                if (item.type === "text")
                    return { type: "text", text: (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_7__/* .sanitizeSurrogates */ .J)(item.text) };
            ...
            for (const block of msg.content) {
                if (block.type === "text") {
                    if (block.text.trim().length > 0) {
                        contentParts.push({ type: "text", text: (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_7__/* .sanitizeSurrogates */ .J)(block.text) });
                    }
                    continue;
                }
                if (block.type === "thinking") {
                    if (block.thinking.trim().length > 0) {
                        contentParts.push({
                            type: "thinking",
                            thinking: [{ type: "text", text: (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_7__/* .sanitizeSurrogates */ .J)(block.thinking) }],
                        });
                    }
                    continue;
                }
        ...
        const textResult = msg.content
            ...
            .map((part) => (part.type === "text" ? (0,_utils_sanitize_unicode_js__WEBPACK_IMPORTED_MODULE_7__/* .sanitizeSurrogates */ .J)(part.text) : ""))
            .join("\n");
